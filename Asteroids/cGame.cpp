...
void cGame::run(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	while (loop)
	{
		...
		//The Rocket Boundaries
		if (theRocket.getSpritePos().x < 0)
		{
			theRocket.setSpritePos({ 0, theRocket.getSpritePos().y });
		}

		if (theRocket.getSpritePos().x > renderWidth -100)
		{
			theRocket.setSpritePos({ renderWidth - 100, theRocket.getSpritePos().y });
		}

		if (theRocket.getSpritePos().y < 0)
		{
			theRocket.setSpritePos({ theRocket.getSpritePos().x, 0 });
		}

		if (theRocket.getSpritePos().y > renderHeight - 100)
		{
			theRocket.setSpritePos({ theRocket.getSpritePos().x, renderHeight - 100 });
		}
